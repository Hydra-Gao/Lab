...
def get_selected_segment_bounds(raw_folder: Path) -> tuple[int, int]:
    ...
    row = match.iloc[0]
    return int(row["segment_start_timestamp_us"]), int(row["segment_end_timestamp_us"])
...
def main() -> None:
    ...
    ttl_raw["segment_start_timestamp_us"] = seg_start_us
    ttl_raw["event_time_sec"] = (
        ttl_raw["timestamp_us"] - seg_start_us
    ) / 1_000_000.0
