...
COLLAPSE_SCREEN_ROLE_FOR_POLAR = True
...
def plot_polar_page(fig, df_speed: pd.DataFrame, speed: str, direction_order: list[float]) -> None:
    raw = collapse_for_polar(df_speed)
    ...
    abs_mean = raw.groupby("direction")["moving_minus_baseline"].mean()

    theta_ratio, ratio_values = close_polar_curve(ratio_mean, direction_order)
    theta_abs, abs_values = close_polar_curve(abs_mean, direction_order)

    ax1 = fig.add_subplot(121, projection="polar")
    ax2 = fig.add_subplot(122, projection="polar")

    scatter_raw_polar_points(ax1, raw, "effect_ratio", direction_order)
    ax1.plot(theta_ratio, ratio_values, color="black", lw=MEAN_POLAR_LINEWIDTH, label="population mean")
    ax1.scatter(theta_ratio, ratio_values, color="black", s=24)
    set_polar_format(ax1, "Relative change\nmean of (moving - baseline) / baseline")

    scatter_raw_polar_points(ax2, raw, "moving_minus_baseline", direction_order)
    ax2.plot(theta_abs, abs_values, color="black", lw=MEAN_POLAR_LINEWIDTH, label="population mean")
    ax2.scatter(theta_abs, abs_values, color="black", s=24)
    set_polar_format(ax2, "Absolute change\nmean of moving - baseline FR")

    fig.suptitle(f"Population polar plots, speed = {speed}", y=0.98, fontsize=14)

    # A single unit legend outside the right panel. For many units this can get tall;
    # reduce fontsize if needed.
...
def main() -> None:
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        # Pages 3-4: one polar page per speed.
        for speed in speed_values:
            df_speed = df.loc[df["speed"].astype(str) == str(speed)].copy()
            fig = plt.figure(figsize=(15, 7.5))
            plot_polar_page(fig, df_speed, speed, direction_order)
            pdf.savefig(fig, bbox_inches="tight")
            png_path = out_dir / f"population_8directions_polar_speed_{str(speed).replace('.', 'p')}.png"
            fig.savefig(png_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
